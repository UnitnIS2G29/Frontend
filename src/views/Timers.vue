<template>
  <v-container>
    ...
    <v-row v-else>
      <v-col cols="12" class="mt-5 mb-5">
        <v-row justify="center" style="margin-top: 50px">
          <v-card v-if="currentTimer">
            <v-card-title>
              <div style="width: 100%; min-width: 500px" justify="center">
                <v-banner justify="center" elevation="12">{{
                  this.elapsedTime
                }}</v-banner>
              </div>
            </v-card-title>
            <v-card-text>
              <v-btn
                color="red"
                @click.stop="editTimerHandler(currentTimer, true)"
                >STOP TIMER</v-btn
              >
            </v-card-text>
          </v-card>
          <v-card v-else>
            ...
            <v-card-text>
              <v-btn color="green" @click.stop="startTimer()"
                >START TIMER</v-btn
              >
            </v-card-text>
          </v-card>
      ...
      <v-col cols="12">
        <h1 class="display-1" @click="loadShowTimers()">
          Timers <v-icon v-if="!showTimers">mdi-arrow-right-bold</v-icon
              ...
              <v-col cols="11">
                <v-card-title>
                  ...
                  <b style="margin-left: 3px">{{
                    timer.category ? "(" + timer.category.name + ")" : ""
                  }}</b>
                </v-card-title>
                <v-card-text>
              ...
              <v-col cols="1">
                <v-card-actions
                  ><v-btn
                    icon
                    color="red"
                    @click.stop="deleteTimerHandler(timer)"
              ...
              <v-col>
                ...
                <v-row>
                  ...
                  <input
                    ...
                    :value="
                      started_at
                        ? moment(started_at).format('YYYY-MM-DDTHH:mm:ss')
                        : null
                    "
                    @change="
                      started_at = moment($event.target.value)
                        .utc()
                        .toISOString()
                    "
                  />
                ...
                <v-row>
                  ...
                  <input
                    ...
                    :value="
                      stopped_at
                        ? moment(stopped_at).format('YYYY-MM-DDTHH:mm:ss')
                        : null
                    "
                    @change="
                      stopped_at = moment($event.target.value)
                        .utc()
                        .toISOString()
                    "
                  />
